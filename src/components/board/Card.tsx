...
import {CardType, CONTENT_TYPES} from "~/types/types";
import {MessageSquareCode, MoreHorizontal} from "lucide-react";
import {EditCardDialog} from "~/components/edit-card/EditCardDialog";
import {useDeleteCardMutation, useUpdateCardOrderMutation} from "~/react-query/mutations";
                    ...
                    {card.labels.length > 0 &&
                        <div className="flex flex-wrap gap-1 mb-3">
                            {card.labels.map((label, idx) =>
                                <Badge key={idx} style={{ backgroundColor: label.color }} className="py-0">
                                    {label.name}
                                </Badge>
                            )}
                        </div>
